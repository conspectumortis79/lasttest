...
test('rejects virtual users outside the supported range', () => {
  equal(validateLoadProfile(0, 10), `Virtual Users müssen zwischen 1 und ${MAX_VIRTUAL_USERS} liegen.`)
  equal(validateLoadProfile(MAX_VIRTUAL_USERS + 1, 10), `Virtual Users müssen zwischen 1 und ${MAX_VIRTUAL_USERS} liegen.`)
})

test('rejects non-integer virtual users and invalid durations', () => {
  equal(validateLoadProfile(1.5, 10), `Virtual Users müssen zwischen 1 und ${MAX_VIRTUAL_USERS} liegen.`)
