...
class DefaultK6ScriptGenerator : K6ScriptGenerator {
     ...
     */
    private fun renderScenario(profile: LoadProfile): String =
        when (profile.type) {
            ...
            LoadProfileType.RAMPING_VUS -> {
                ...
                val stagesLiteral = stages.joinToString(", ") { stage ->
     ...
     * error.
     */
    private fun validateLoadProfile(profile: LoadProfile) {
        when (profile.type) {
            LoadProfileType.CONSTANT_VUS -> {
            ...
            LoadProfileType.RAMPING_VUS -> {
                ...
                for ((index, stage) in stages.withIndex()) {
                    ...
                    // Two consecutive stages with the same target would emit
                    // a no-op plateau that confuses readers of the report.
                    require(stage.target != previousTarget || index == stages.lastIndex) {
                        "Stage ${index + 1}: aufeinanderfolgende Stages dürfen nicht dasselbe Ziel haben."
                    }
            ...
            LoadProfileType.CONSTANT_ARRIVAL_RATE -> {
                ...
                require(timeUnit in ALLOWED_TIME_UNITS) { "Zeiteinheit muss eine Sekundenzahl zwischen 1 und 60 sein." }
                require(duration in 1..MAX_DURATION_SECONDS) { "Die Dauer muss zwischen 1 und $MAX_DURATION_SECONDS Sekunden liegen." }
