...
class LocalK6TestRunService(
     ...
     * legacy triple we synthesise a matching [LoadProfile] so old callers
     * keep working while they migrate. This is the single point where
     * `useIterations`, `virtualUsers` and `durationSeconds` are read on
     * the request side — once the profile is resolved, the rest of the
     * pipeline only sees the new shape.
     */
    private fun resolveLoadProfile(request: CreateTestRunRequest): LoadProfile {
        ...
        val useIterations = request.useIterations ?: false
        require(vus != null && duration != null) {
            "Es wurde weder loadProfile noch das legacy-Tripel (virtualUsers, durationSeconds) übergeben."
        }
        return if (useIterations) {
