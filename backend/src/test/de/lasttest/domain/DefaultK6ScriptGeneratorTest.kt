...
class DefaultK6ScriptGeneratorTest {
    ...
    @Test
    fun `rejects ramping-vus with consecutive equal targets`() {
        val profile =
            LoadProfile(
                type = LoadProfileType.RAMPING_VUS,
                stages = listOf(LoadStage(10, 30), LoadStage(10, 30)),
            )
        val exception =
            assertFailsWith<IllegalArgumentException> {
                generator.generate(specification, "https://example.test", setOf("getPet"), emptyList(), profile)
            }
        assertTrue(exception.message!!.contains("aufeinanderfolgende"))
    }
